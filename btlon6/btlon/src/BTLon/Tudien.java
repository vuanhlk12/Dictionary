...
public class Tudien {
	...
	public void Ssearch(String a) {
		int i;
		for (i = 0; i < this.getSoluong(); i++) {
			if (this.words[i].tu.startsWith(a)) {
	...
	public void Ssearch2(String a) {
		int i;
		for (i = 0; i < Tudien.word.size(); i++) {
			if (Tudien.word.get(i).startsWith(a)) {
				String resulf1 = Tudien.word.get(i);
				String resulf = Tudien.hMapEV.get(resulf1);
